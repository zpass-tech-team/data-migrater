...
public class HibernateConfig extends HibernateDaoConfig {
    ...
    private static final String SCHEMA_NAME = "MOSIP";
    ...
    static {
        try (InputStream configKeys = HibernateConfig.class.getClassLoader().getResourceAsStream("database.properties")) {
            keys = new Properties();
            keys.load(configKeys);
        } catch (Exception e) {
            LOGGER.error(LOGGER_CLASS_NAME, APPLICATION_NAME, APPLICATION_ID,
                    "Exception encountered during context initialization - HibernateConfig "
 ...
 //       jdbcTemplate();
    ...
    private void isKeySet(Statement statement, String key, String value) throws Exception {
        ...
        throw new Exception((key.startsWith("derby.user") ? "derby.user.*" : key) +
                " : is not set to preferred value!");
    }

    private boolean createDb(String dbPath) throws Exception {
        ...
        throw new Exception(RegistrationExceptionConstants.APP_INVALID_STATE.getErrorCode() +
    ...
    public void updateGlobalParamsInProperties(JdbcTemplate jdbcTemplate) {
        if (!isPPCUpdated) {
            ...
            try {
                dbConfig = getDBConf();
                properties.put("spring.datasource.url", String.format(URL + ";create=true;" + ENCRYPTION_URL_ATTRIBUTES, dbPath, dbConfig.get(BOOTPWD_KEY)));
                properties.put("spring.datasource.username", dbConfig.get(USERNAME_KEY));
                properties.put("spring.datasource.password", dbConfig.get(PWD_KEY));
            } catch (IOException e) {
                e.printStackTrace();
            }
