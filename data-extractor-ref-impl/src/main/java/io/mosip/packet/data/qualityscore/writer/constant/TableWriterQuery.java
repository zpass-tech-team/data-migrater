...
public class TableWriterQuery {
    ...
    public TableWriterQuery() {
        ...
        inserQuerytMap.put(DBTypes.ORACLE, "MERGE INTO <TABLE_NAME> T USING (SELECT 1 FROM DUAL) C ON (T.REF_ID = '<REF_ID>') WHEN NOT MATCHED THEN INSERT (<COLUMN_NAME>) values(<VALUES>) WHEN MATCHED THEN UPDATE SET <UPDATE_COLUMN_NAME>");
